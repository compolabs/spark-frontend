...
import { AssetIdInput, I64Input } from "./types/account-balance/AccountBalanceAbi";
...
export class Api {
  ...
  cancelSpotOrder = async (orderId: string, wallet: WalletLocked | WalletUnlocked): Promise<void> => {
    const orderbookFactory = OrderbookAbi__factory.connect(CONTRACT_ADDRESSES.spotMarket, wallet);

    await orderbookFactory.functions.cancel_order(orderId).txParams({ gasPrice: 1, gasLimit: GAS_LIMIT }).call();
  };
  ...
  depositPerpCollateral = async (assetAddress: string, amount: string, wallet: WalletLocked | WalletUnlocked) => {
  ...
  removePerpOrder = async (orderId: string, wallet: WalletLocked | WalletUnlocked): Promise<void> => {
    const clearingHouseFactory = ClearingHouseAbi__factory.connect(CONTRACT_ADDRESSES.clearingHouse, wallet);
