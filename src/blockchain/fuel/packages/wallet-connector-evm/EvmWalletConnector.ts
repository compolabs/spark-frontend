...
import { ConnectorMetadata, FuelConnector, Network, Version } from "@fuel-wallet/sdk";
import { AbiMap, Asset } from "@fuel-wallet/types";
import {
  Address,
  getPredicateRoot,
  InputValue,
...
export class EVMWalletConnector extends FuelConnector {
  ...
  async sendTransaction(address: string, transaction: TransactionRequestLike): Promise<string> {
    ...
    const predicate = createPredicate(account.ethAccount, fuelProvider, this.predicate.bytecode, this.predicate.abi);
    ...
    const transactionWithPredicateEstimated = await fuelProvider.estimatePredicates(requestWithPredicateAttached);
  ...
  async currentAccount(): Promise<string | null> {
    ...
    const fuelAccount = getPredicateAddress(ethAccounts[0]!, this.predicate.bytecode, this.predicate.abi);
  ...
  > {
    ...
    const accounts = ethAccounts.map((account) => ({
      ethAccount: account,
      predicateAccount: getPredicateAddress(account, this.predicate.bytecode, this.predicate.abi),
    }));
    return accounts;
  }
}
...
export const getPredicateAddress = memoize(
  (ethAddress: string, predicateBytecode: BytesLike, predicateAbi: JsonAbi): string => {
    const configurable = {
      SIGNER: Address.fromB256(ethAddress.replace("0x", "0x000000000000000000000000")).toEvmAddress(),
    };

    // @ts-ignore
    const { predicateBytes } = Predicate.processPredicateData(predicateBytecode, predicateAbi, configurable);
...
export const createPredicate = memoize(function createPredicate(
  ...
  const predicate = new Predicate(arrayify(predicateBytecode), provider, predicateAbi, configurable);
